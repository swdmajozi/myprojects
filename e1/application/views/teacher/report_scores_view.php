...
<aside class="right-side">
	...
	<section class="content-header">
		<h1>
			<span class="fa fa-bar-chart-o"></span> report
			<small></small>
		</h1>
		<ol class="breadcrumb">
							...
							<div class="col-sm-6" style="z-index:500;">
								...
								<label><?php
									$options = array(
										...
										array(
											'all' => 'Science And Liberal Arts (all)',
				...
				<table class="table table-striped table-hover textvertmid rowclick">
					<thead>
						<tr>
							...
							<th style="width: 80px;" class="">Open Exam#</th>
						</tr>
					</thead>
					<tbody>
					<?php
						if ($courseslist) {
							foreach ($courseslist as $item) {
								...
								<td class="status">
									<div class="btn-group">
										<a href="{$this->misc->getHref('teacher/reports/bypaper')}/{$item['course_id']}" class="btn btn-sm btn-default btn-flat">Classified by examination</a>
										<button type="button" class="btn btn-sm btn-default btn-flat dropdown-toggle" data-toggle="dropdown">
											<span class="caret"></span>
											<span class="sr-only">Toggle Dropdown</span>
										</button>
										<ul class="dropdown-menu" role="menu">
											<li><a href="{$this->misc->getHref('teacher/reports/bystudent')}/{$item['course_id']}">Classified By person</a></li>
										</ul>
									</div>
								</td>
								<td>{$year}</td>

	...
	$("#chapterAdd").on("click", function(e) {
		...
		.fail(function(jqxhr, textStatus, error) {
			...
			var jbox = new jBox(\'Modal\', {
				...
				title: \'An error\',
				overlay: true,
				content: \'Cant addบทได้ กรุณาตรวจสอบความถูกต้อง\',
			});
			jbox.open();
	...
	function callbackjson()
	{
		...
		switch ($arg_list[0]) {
			...
			case 'addQuestion':
				...
				$msgerr = "Cant add due to the following error<br><ul>";
				$errObj = array();

				if ($chapter_id == "" || !is_numeric($chapter_id))
				{
					$msgerr .= "<li>Chapter ID information not found or invalid.</li>";
				}
				...
				{
					$msgerr .= "<li>Question type not found or invalid</li>";
					$errObj['qtype'] = 1;
				}
				if (trim(strip_tags($question)) == "")
				{
					$msgerr .= "<li>Must have question questions for test takers</li>";
					$errObj['question'] = 1;
				}
				if ($answer == "")
				{
					$msgerr .= "<li>Must have the correct answer</li>";
					$errObj['correct'] = 1;
				}
				...
				switch ($question_type) {
					case 'choice':
						...
						{
							$msgerr .= "<li>Multiple choice: Must have at least 2 options</li>";
							$errObj['choices'] = 1;
						}
						if ($answer != "" || is_numeric($answer))
						{
							...
							{
								$msgerr .= "<li>Multiple choice: the right choice Do not enter an answer!</li>";
								$errObj['choices'] = 1;
						...
						{
							$msgerr .= "<li>Multiple choice: must choose the correct answer</li>";
							$errObj['choices'] = 1;
						}
						break;
					case 'numeric':
						...
						{
							$msgerr .= "<li>Fill in numbers: The correct answer must be specified with numbers.</li>";
							$errObj['correct'] = 1;
						}
						break;
					case 'boolean':
						...
						{
							$msgerr .= "<li>True / False: Choose the correct answer as right or wrong.</li>";
							$errObj['correct'] = 1;
						}
						break;

					default:
						$msgerr .= "<li>Invalid Question type condition</li>";
						break;
				}

				$msgerr .= "</ul>";

				if ($msgerr != "Cant add because of the following error<br><ul></ul>")
				{
					echo json_encode(array(
						'error' => $msgerr,
						'objects' => $errObj
		...
		else
		{
			...
			else
			{
				...
				if (!empty($data['subjectInfo']))
				{
					...
					$data['pagetitle'] = "Manage Exam Bank";
					$data['pagesubtitle'] = "Subject ".$data['subjectInfo']['code']." ".$data['subjectInfo']['name'];

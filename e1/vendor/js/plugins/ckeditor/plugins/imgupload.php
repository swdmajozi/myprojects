...
if(isset($_FILES['upload']) && strlen($_FILES['upload']['name']) > 1) {
  ...
  $site = $protocol. $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];

  $uploadpath = dirname(__FILE__) .'/'. $upload_dir . $img_name;       // full file path
  ...
  if($err == '') {
    if(move_uploaded_file($_FILES['upload']['tmp_name'], $uploadpath)) {
      ...
      $url = dirname($site).'/'. $upload_dir . $img_name;

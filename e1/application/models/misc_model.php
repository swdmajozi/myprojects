...
class Misc_model extends CI_Model {
	...
	function getErrorDesc($errno,$mode='')
	{
		switch ($errno) {
			case 1062:
				if ($mode=="user")
					return "Username already exists Cannot be repeated";
				else
					return "Duplicate";
				break;
	...
	function getFullDateTH($strdate)
	{
		...
		$ThaiDate = "day".$thai_day_arr[date("w",$strdate)];
		$ThaiDate.= "hours ".date("j",$strdate);
		$ThaiDate.= "%m ".$thai_month_arr[date("n",$strdate)];
		$ThaiDate.= " %y".(date("Y",$strdate)+543);
	...
	function buildYearOptions($range = '1')
	{
		$startYear = date("Y");
		$year = $startYear;
		for($i = 0;$i < $range;$i++)
		{
			$options[$year] = $year + 000;
			$year--;
		}
		return $options;
	}
	...
	{
		$options['1'] = "Semester 1";
		$options['2'] = "Semester 2";
		return $options;
	}
	...
	function getActiveStatusIcon($str)
	{
		switch ($str) {
			case 'active':
				return '<i class="text-green fa fa-circle jtooltip" title="Enable"></i>';
				break;

			case 'inactive':
				return '<i class="fa fa-circle-o jtooltip" title="Disabled"></i>';
				break;
	...
	function getVisibilityStatusIcon($str)
	{
		switch ($str) {
			...
			case 1:
			case '1':
				return '<i class="fa fa-eye jtooltip" title="Everyone can see"></i>';
				break;
			...
			case 0:
			case '0':
				return '<i class="fa fa-eye-slash jtooltip" title="To hide"></i>';
				break;
